...
def init_wizard(*, local: bool) -> int:
    ...
    try:
        ...
        # (especially important for global config).
        ...
        if provider == "ollama":
            ...
            if not models:
                _eprint("No Ollama models found.")
            ...
            _eprint(f"Unsupported provider: {provider}")
            return 2
        ...
        if use_conventional:
            ...
            scope_strategy: ScopeStrategy = questionary.select(
                ...
                choices=[
                    questionary.Choice(
                        "🤖 Auto-detect scope from changes", value="auto"
                    ),
        ...
        # Questionary prints its own cancellation message ("Cancelled by user").
        return 130
